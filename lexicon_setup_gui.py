	...
	while not done:
		...
		if screenCounter == numTeams + 3:
			...
			maxRoundsInput = inputLabel(surface, col1, row1+lineSpacing, col2, row1+lineSpacing, text='Number of rounds:', \
							font=niceFont, highlight_colour=hotPink, cursorColour=cursorColour)
			inputCounter = 0
			paramInputs = [timeInput, maxRoundsInput]
				...
				for event in events:
					if event.type == pygame.KEYDOWN:
						if event.key == pl.K_UP:
							...
							inputCounter += 1
							if inputCounter % 2 == 0 and (inputCounter != 0):
								try:
									timeLimit = int(timeInput.get_input())
									maxRounds = int(maxRoundsInput.get_input())
									screenCounter += 1
								except ValueError:
									errorTxt.string = 'Please enter an integer number for time and rounds.'
		...
		if screenCounter == 4 + numTeams:
			...
			dictionaries = [x for x in os.listdir(cwd + '/wordLists') if x.endswith('list')]
			dictionaries.sort()
			options = []
			rowIndex = 0
			inputCounter = 0
			for option in dictionaries:
				options.append(selector(surface, col1, row1 + (lineSpacing*rowIndex), col2,\
										row1 + (lineSpacing*rowIndex), text=option[:-5], font=niceFont, box_colour=hotPink))
				rowIndex += 1

			def next_():
				global screenCounter
				global chosen_dictionaries
				chosen_dictionaries = [cwd + '/wordLists/' + x.text + '.list' for x in options if x.selected == True]
				screenCounter += 1

			nextButton = button(surface, 710, 500, 50, 20, 'rrect', text='START', font=buttonFont, function=next_, colour = buttonColour)
			while screenCounter == 4 + numTeams:
				events = pygame.event.get()
				surface.fill(bgColour)
				for i in range(len(options)):
					if i == (inputCounter % len(options)):
						options[i].active = True
					else:
						options[i].active = False
					options[i].draw(events)
				for event in events:
					if event.type == pygame.KEYDOWN:
						if event.key == pl.K_UP:
							inputCounter -=1
		...
		if screenCounter == 5 + numTeams:	
			return teams, chosen_dictionaries, timeLimit, maxRounds
